...
* **تخصيص الذاكرة على المكدس (Stack Allocation):** معالجة السلاسل النصية وتشكيلها وعكسها داخل مكدس الذاكرة C بدون حجز ديناميكي في المسارات الحرجة مع تعطيل قفل بايثون (`nogil`).
* **فحص الأحرف بجداول البتات (64-bit Bitmaps):** تصنيف الرموز والحركات والتشكيل بتعقيد زمني ثابت $O(1)$ عبر عمليات بتية مباشرة في دورة معالج واحدة.
* **إنشاء مباشر للنصوص عبر C-API:** بناء السلاسل النصية مباشرة عبر واجهة C وتفادي الاستهلاك الزائد لكائنات بايثون الوسيطة.
* **قراءة الملفات عبر خرائط الذاكرة (`mmap`):** وصول فوري بدون نسخ للبيانات مع معالجة متعددة العمليات (Multi-Processing) لتجاوز قفل GIL للملفات الضخمة.
* **الحفاظ الذكي على الأكواد والتنسيقات:** تخطي تلقائي للروابط والمسارات والأرقام وكتل الأكواد البرمجية (Code Blocks) وعلامات الترقيم وقوائم Markdown.

### المميزات الأساسية
* **تشكيل متصل دقيق للكلمات:** تحويل الحروف لسياقها الصحيح (Unicode Presentation Forms-B) مع دعم كامل وتلقائي لجميع أشكال اللام ألف (`لا`، `لأ`، `لإ`، `لآ`) ولفظ الجلالة (`ﷲ`).
* **دعم التشكيل والحركات المركبة:** الحفاظ على الحركات وعلامات الضبط المصحفي دون كسر اتصال الحروف، مع خيار إزالة الحركات لتنسيق شبكة الأحرف بدقة.
* **وضع الخادم السريع (Daemon Mode):** خادم خلفي عبر Unix domain sockets لاستجابة فورية بأقل من ملّي ثانية دون أي تأخير لبدء بايثون (يتوقف تلقائياً بعد 5 دقائق من الخمول).
* **واجهة سطر أوامر ومكتبة برمجية شاملة:** دعم الأنابيب (`stdin/stdout`)، والمدخلات المباشرة، ومعالجة الدُفعات والملفات، والاستدعاء المباشر عبر لغة بايثون.
* **محرك بديل تلقائي (Zero-Config Fallback):** يعمل بسلاسة عبر محرك بايثون نقي في حال عدم توفر المترجم المدمج.
...
# معالجة الملفات مع تعدد العمليات
arabic-rtl -f input.txt -o output.txt -t 4

# تشغيل اختبار قياس الأداء المدمج
arabic-rtl --benchmark
```

#### 4. وضع الخادم اللحظي (Daemon Mode)
```bash
# تشغيل الخادم في الخلفية
arabic-rtl-daemon start

# تمرير الطلبات عبر الخادم النشط
echo "نص عربي سريع جداً" | arabic-rtl --daemon

# التحقق من الحالة أو إيقاف الخادم
arabic-rtl-daemon status
arabic-rtl-daemon stop
```

#### 5. الاستخدام عبر كود بايثون (Python API)
```python
...
### استخدام الأداة كمهارة للذكاء الاصطناعي (AI Assistant Skill)
...
### خيارات ومعاملات سطر الأوامر (CLI Options & Flags)

| المعامل المختصر | المعامل الطويل | الوصف |
|-----------------|----------------|-------|
| `-f` | `--file PATH` | مسار ملف الإدخال (يستخدم `mmap` للملفات الكبيرة) |
| `-o` | `--output PATH` | مسار ملف الإخراج (الافتراضي: `stdout`) |
| `-t` | `--threads N` | عدد عمليات المعالجة المتوازية (الافتراضي: تلقائي) |
| `-d` | `--daemon` | توجيه الطلب عبر خادم `arabic-rtl-daemon` النشط |
| `-s` | `--show-stats` | طباعة إحصائيات التوقيت والسرعة وحجم البيانات |
| `-b` | `--benchmark` | تشغيل حزمة قياس الأداء واختبارات السرعة |
| `-kt` | `--keep-tashkeel` | إبقاء التشكيل والحركات في النص الناتج |
| `-nt` | `--no-tashkeel` | حذف كافة الحركات وعلامات التشكيل |
| `-al` | `--allah-ligature` | تحويل لفظ الجلالة `الله` إلى المحرف الموحد `ﷲ` (U+FDF2) |
| `-S` | `--stream` | معالجة وطباعة المخرجات سطراً بسطر فور قراءتها |
| `-r` | `--rtl` | إخراج النص بتنسيق محاذاة RTL للطرفية |
